...
def EdificeWidgets(self):
    config_init: typing.Optional[ac.Config] = None
    ...
    def on_load_config_button_click(e):
        ...
        c = ac.parse_config_file(file_path)
        print(c)
        set_config(c)

    def on_new_config_button_click(e):
        set_config(ac.Config())
...
def create_config_view(config: ac.Config) -> QWidget:
    w = QWidget()
...
if __name__ == "__main__":
    ...
    current_config: ac.Config | None = None
    config_widget: QWidget | None = None
    def set_config(config: ac.Config):
    ...
    def on_load_config():
        ...
        file_path = file_name[0]
        try:
            config = ac.parse_config_file(file_path)
            set_config(config)
        except ValueError:
